Function TableSelectListByRef(e) ' [KEYWORD=TRUE] '
						...
						For iCounter = 0 To objObject.RowCount
							...
							Case "WebTable"
								If Trim(UCase(objObject.GetCellData(iCounter, intCheckCol))) = Trim(UCase(strFindValue)) Then
									If Trim(UCase(strSetValue)) <> "%SKIP%" Then objObject.ChildItem(iCounter, strTargetColumn, strChildClass, intChildIndex).Select strSetValue
									Exit For
								End If
							Case "JavaTable"
'Add for JavaTable
					...
					Else
						...
						For iCounter = 0 To UBound(tmpTabArr, 1)
							If Trim(UCase(tmpTabArr(iCounter, strTargetColumn))) = Trim(UCase(strFindValue)) Then
								Set tmpObj = Browser("CreationTime:=" & myPactera_commonFunction_getActiveBrowserIndex).Page("index:=0").WebList("visible:=True", "index:=0", "innertext:=" & tmpTabArr(iCounter, strTargetColumn))
								If tmpObj.Exist(0) Then
									On Error Resume Next
									If Trim(UCase(strSetValue)) <> "%SKIP%" Then tmpObj.Select strSetValue
									Exit For
								Else
				...
				Else
					If objObject.GetROProperty("cols") > 1 Then
						...
						Select Case stepReference.GetTOProperty("micclass")
						Case "WebTable"
							objObject.ChildItem(intRow, strTargetColumn, strChildClass, intChildIndex).Select strSetValue
						Case "JavaTable"
'Add for JavaTable
						End Select
					Else
						tmpTabArr = myPactera_commonFunction_extractInnerHTMLTable(objObject.GetROProperty("innerhtml"))
						Set tmpObj = Browser("CreationTime:=" & myPactera_commonFunction_getActiveBrowserIndex).Page("index:=0").WebList("visible:=True", "index:=0", "innertext:=" & tmpTabArr(intRow, strTargetColumn))
		...
		End If
	End If	
	Set TableSelectListByRef = EvaluateExecution(Err, "")
	Err.Clear: On Error GoTo 0
End Function